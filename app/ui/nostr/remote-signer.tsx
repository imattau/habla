...
export default function RemoteSignerLogin({
  ...
  const RELAYS = useMemo(
    () => ["wss://relay.nsec.app", ...AGGREGATOR_RELAYS],
    [],
  );
    ...
    <div className="grid gap-4 py-4">
      <Tabs defaultValue="bunker" className="w-full">
        ...
        <TabsContent value="qr">
          <QRCodeFlow relays={RELAYS} onConnected={onConnected} />
        </TabsContent>
      </Tabs>
    </div>
  );
  ...
  useEffect(() => {
      ...
      return pool.req(relays, filters).pipe(
        tap((ev) => {
          if (typeof ev === "string") {
            ...
            console.log(
              "[RemoteSigner:QR] Received Nostr event:",
              ev.kind,
              ev.id,
            );
          }
    ...
    const finish = async () => {
      if (finishedRef.current || abortedRef.current) {
        console.log(
          "[RemoteSigner:QR] finish() bailing. finished:",
          finishedRef.current,
          "aborted:",
          abortedRef.current,
        );
      ...
      try {
        ...
        if (abortedRef.current) {
          console.log(
            "[RemoteSigner:QR] Public key fetched but instance already aborted.",
          );
          return;
        }
        ...
        console.log(
          "[RemoteSigner:QR] Login successful. Calling onConnected()...",
        );
    ...
    (signer as any).handleEvent = async (event: any) => {
      console.log(
        "[RemoteSigner:QR] signer.handleEvent called for kind",
        event.kind,
      );
      try {
        await originalHandleEvent(event);
        console.log(
          "[RemoteSigner:QR] signer.handleEvent finished. isConnected:",
          signer.isConnected,
        );
        if (signer.isConnected && !finishedRef.current && !abortedRef.current) {
          console.log(
            "[RemoteSigner:QR] isConnected is true! Triggering finish()...",
          );
          finish();
        }
      } catch (e) {
